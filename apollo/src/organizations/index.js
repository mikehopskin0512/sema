...
export default (app, passport) => {
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (action === 'toggleOrganizationCollection') {
          ...
          if (
            !_checkPermission(organizationData._id, 'canEditCollections', user)
          ) {
  ...
  route.post(
    ...
    async (req, res) => {
      try {
        ...
        const userRole = await updateOrganizationAvatar(
          organizationId,
          userId,
          req.file
        );
  ...
  route.get(
    ...
    async (req, res) => {
      try {
        ...
        const repos = await getOrganizationRepos(organizationId, {
  ...
  route.put(
    ...
    async (req, res) => {
      try {
        ...
        const organization = await updateOrganizationRepos(
          organizationId,
          repos
        );
        return res.status(200).send(organization);
  ...
  route.get(
    ...
    async (req, res) => {
      try {
        const { organizationId } = req.params;
        const [metrics, totalMetrics] = await Promise.all([
          getOrganizationMetrics(organizationId),
          getOrganizationTotalMetrics(organizationId),
        ]);
        return res.send({ metrics, totalMetrics });
      } catch (error) {
        logger.error(error);
        return res.status(error.statusCode).send(error);
      }
  ...
  route.get(
    ...
    async (req, res) => {
      try {
        ...
        const { members, totalCount } = await getOrganizationMembers(
          organizationId,
          {
      ...
      try {
        ...
        const result = await Promise.all(
          users.map(async (email) => {
            ...
            if (semaUser) {
              const isOrganizationMember = await UserRole.exists({
        ...
        return res
          .status(200)
          .send({ invalidEmails: result.filter((item) => !!item) });
      } catch (error) {
        logger.error(error);
        return res.status(error.statusCode).send(error);
      }
      ...
      try {
        ...
        const invites = await Promise.all(
          users.map(async (email) => {
            ...
            const { recipient, token, orgName, senderName, senderEmail } =
              newInvitation;
        ...
        return res
          .status(200)
          .send({ invalidEmails: invites.filter((item) => !!item) });
      } catch (error) {
        logger.error(error);
        return res.status(error.statusCode).send(error);
      }
  ...
  route.get(
    ...
    async (req, res) => {
      try {
        ...
        const result = await addOrganizationMembers(
          organizationId,
          [username],
          role
        );
