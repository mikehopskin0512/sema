...
export default (app) => {
  ...
  route.post('/github', async (req, res) => {
    try {
      if (['pull_request_review_comment', 'pull_request_review', 'issue_comment'].includes(req.headers['x-github-event'])) {
  ...
  // Swagger route
  app.use(`/${version}/webhooks-docs`, checkEnv(), swaggerUi.serveFiles(swaggerDocument, {}), swaggerUi.setup(swaggerDocument));
};
