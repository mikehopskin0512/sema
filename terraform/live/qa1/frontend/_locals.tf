locals {
  ...
  apollo_worker_secret = "${var.name_prefix}-apollo-worker-secret"
...
locals {
  ...
  apollo_worker_ecs_secrets = flatten([
    {
      envs      = keys(jsondecode(nonsensitive(data.aws_secretsmanager_secret_version.apollo_worker.secret_string)))
      valueFrom = aws_secretsmanager_secret.apollo_worker.arn
      kms       = try(aws_secretsmanager_secret.apollo_worker.kms_key_id, null)
    }
  ])
  ...
  apollo_worker_ecs_secret_data_hash = nonsensitive(sha256(data.aws_secretsmanager_secret_version.apollo_worker.secret_string))
  ...
  datadog_apollo_worker_api_key = try(jsondecode(nonsensitive(data.aws_secretsmanager_secret_version.apollo_worker.secret_string))["DATADOG_API_KEY"], null)
}
