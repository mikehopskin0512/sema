...
const CollaborationPublicPage = () => {
  ...
  const { ref: graphWrapperRef } = useResizeObserver();
  const {
    handle,
    repoId,
    repos
        ...
        <div className={styles['collaboration-bottom-control']}>
          <div className={styles['collaboration-control-left-block']}>
            <div>
              <img style={{
                width: '104px',
                height: '104px',
              }} alt='Happy' src='/img/happy.png' />
            </div>
            <p className='has-text-white-0' style={{ maxWidth: '65%' }}>
              Larger, closer circles are your tight-knit team, who you worked with the most over the last year. Smaller, distant circles are team
              members you don’t spend as much time working with.
            </p>
          </div>
          <div className={styles['collaboration-control-block']} style={{ zIndex: 10 }}>
            ...
            <div className='is-flex is-justify-content-left is-full-width is-flex-wrap-wrap' style={{ paddingLeft: '5%' }}>
              ...
              <a target='_blank' href={`${SEMA_INTERCOM_URL}${SEMA_FAQ_SLUGS.LEARN_MORE_ABOUT_SOCIAL_CIRCLES}`}
                 className={styles['collaboration-side-button']}>Learn more</a>
            </div>
          </div>
        </div>
      </div>
