...
function UserHeaderNav({ toggleHamburger, type = 'desktop', selectedOrganization }) {
  ...
  const HEADER_BACKGROUND = constants.headerBackground.split(', ').map((item) => item.replace(/[\[\]']+/g,''));
  ...
  const handleOnOrganizationClick = (org) => {
    setOrganization(org);
    router.push(org ? `${PATHS.ORGANIZATIONS._}/${org?.organization?._id}${PATHS.REPOS}` : PATHS.DASHBOARD);
  };

  const toggleOrgSwitcherItemsHoverColor = (e) => {
    const el = e.target;
    el.classList.toggle(isEmpty(selectedOrganization) ? 'org-colour-personal' : `org-colour-${orgIndex % 15}`);
  }

  const renderOrgName = () => isEmpty(selectedOrganization) ? `${userData.firstName} ${userData.lastName}` : selectedOrganization.organization.name;
  const renderAvatar = () => isEmpty(selectedOrganization) ? userData.avatarUrl : selectedOrganization.organization.avatarUrl;
  ...
  useEffect(() => {
    if(typeof selectedOrganization === 'object' && organizations.organizations.length > 0) {
      setOrgIndex(organizations.organizations.findIndex((element) => element._id === selectedOrganization._id));
    }
  ...
  const renderDesktopNav = () => (
    <>
      {organizations.organizations.length > 0 && 
        <div 
          className={clsx('border-radius-4px is-flex is-justify-content-center is-align-items-center mr-20', 
            styles['external-org-switcher-container'],
            isEmpty(selectedOrganization) ? 'org-colour-personal' : `org-colour-${orgIndex % 15}`
          )}
        ...
        > 
          <div className='is-flex'>
            <Avatar
              ...
              round
            />
            <span className={clsx('has-text-black-950 has-text-weight-semibold is-size-7 px-8', styles['selected-org-name'])}>
              {renderOrgName()}
            </span>
            {openOrgSwitcher ? <ArrowDropupIcon className={clsx(styles['dropdown-icon'])} /> : <ArrowDropdownIcon className={clsx(styles['dropdown-icon'])} />}
          </div>
          {openOrgSwitcher && 
            <div className={clsx(styles['internal-org-switcher-container'], 'has-background-white border-radius-4px')}>
              ...
              <div className={clsx(styles['org-switcher-item'], 
                ...
                <div className='is-flex is-align-items-center noHover'>
                  ...
                  <div className='is-flex is-flex-direction-column'>
                    <div className='is-flex is-align-items-center'>
                      ...
                      <p className={clsx('has-text-black-950 has-text-weight-semibold', styles['user-account-name'])}>
                        {`${userData.firstName} ${userData.lastName}`}
                      </p>
                    </div>
                    <p className='is-size-8'>{repositories.data.repositories.length} repos</p>
                  </div>
                </div>
              </div>
              {organizations.organizations.map((org, idx) => (
                <div key={org._id}
                  className={clsx('is-flex is-flex-direction-column is-justify-content-center', 
                ...
                >
                  <div className='is-flex is-align-items-center noHover'>
                    ...
                    <div className='is-flex is-flex-direction-column'>
                      <p className='has-text-black-950 has-text-weight-semibold'>{org.organization.name}</p>
                      <p className='is-size-8'>{org.organization.repos.length} repos</p>
                    </div>
                  </div>
                </div>
              ))}
              ...
              >
                <Link href={PATHS.ORGANIZATION_CREATE} className='is-flex is-align-items-center is-justify-content-center noHover'>
                  <span className="ml-30 has-text-blue-700 has-text-weight-semibold">+ Add another GitHub Org</span>
                </Link>
              </div>
            </div>
          }
        ...
        renderMenuItems(personalDashboard, isEmpty, selectedOrganization).map((item, idx) =>
          <Link key={idx} href={item.path}>
            <a
              ...
              style={setMenuItemStyle(item.stylePath ? item.stylePath : item.path)}
              onMouseEnter={toggleOrgSwitcherItemsHoverColor}
              onMouseLeave={toggleOrgSwitcherItemsHoverColor}
            >
              {item.title}
